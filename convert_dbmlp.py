#!/usr/bin/env python3
"""
DBMLP to SAA-C03 JSON Converter

This script converts DBMLP format JSON files to SAA-C03 format.
It reads all JSON files from DBMLP directory and merges them into a single SAA-C03 format file.
"""
...
def convert_dbmlp_question_to_saa_c03(question_data: Dict[str, Any], question_number: int) -> Dict[str, Any]:
    """
    Convert a single DBMLP question to SAA-C03 format
    """
...
def load_dbmlp_file(file_path: str) -> List[Dict[str, Any]]:
    """
    Load and parse a DBMLP JSON file
    """
    try:
...
def convert_all_dbmlp_files(input_dir: str, output_file: str):
    """
    Convert all DBMLP JSON files to SAA-C03 format
    """
    ...
    for file_path in json_files:
        ...
        questions = load_dbmlp_file(file_path)
        
        # Convert each question
        for question_data in questions:
            try:
                converted_question = convert_dbmlp_question_to_saa_c03(question_data, question_number)
...
def main():
    ...
    Main function
    """
    input_directory = "DBMLP"
    output_file = "DBMLP.json"
    ...
    print(f"Converting DBMLP files from '{input_directory}' to '{output_file}'")
    
    # Perform the conversion
    convert_all_dbmlp_files(input_directory, output_file)
